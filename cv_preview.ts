...
function openOptionsModal() {
    const modal = document.getElementById('options_modal') as HTMLElement;
    modal.style.display = 'block';
}

function downloadAsPDF() {
    const doc = new jsPDF();
    const profilePictureURL = (document.getElementById('preview_profile_picture') as HTMLImageElement).src;
    const name = (document.getElementById('preview_name') as HTMLElement).textContent || '';
    const email = (document.getElementById('preview_email') as HTMLAnchorElement).textContent || '';
    const linkedin = (document.getElementById('preview_linkedin') as HTMLAnchorElement).textContent || '';
    const github = (document.getElementById('preview_github') as HTMLAnchorElement).textContent || '';
...
function getShareableURL() {
    ...
    const params = new URLSearchParams({
        name: (document.getElementById('preview_name') as HTMLElement).textContent || '',
        email: (document.getElementById('preview_email') as HTMLAnchorElement).textContent || '',
        linkedin: (document.getElementById('preview_linkedin') as HTMLAnchorElement).textContent || '',
        github: (document.getElementById('preview_github') as HTMLAnchorElement).textContent || '',
        work_experience: (document.getElementById('preview_work_experience') as HTMLElement).textContent || '',
        education: JSON.stringify([...document.querySelectorAll('#preview_education li')].map(item => (item as HTMLElement).textContent || '')),
        skills: JSON.stringify([...document.querySelectorAll('#preview_skills li')].map(item => (item as HTMLElement).textContent || ''))
    });
